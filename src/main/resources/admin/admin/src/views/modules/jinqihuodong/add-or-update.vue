    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="活动简介" prop="huodongjianjie">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.huodongjianjie" label="活动简介" prop="huodongjianjie">
                    <span v-html="ruleForm.huodongjianjie"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.zhuangtaiOptions = "全部状态,报名中,已结束".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jinqihuodong/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.huodongjianjie = this.ruleForm.huodongjianjie.replace(reg,'../../../springboot68bv8/upload');
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jinqihuodong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jinqihuodongCrossAddOrUpdateFlag = false;
                  this.parent.search();
